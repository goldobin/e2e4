...
typedef enum {
    BOARD_STATE_PARSE_ERR_OK = 0,
    BOARD_STATE_PARSE_ERR_INVALID_VALUE,
} BoardStateParseErr;

typedef struct {
    BoardStateParseErr err;
    size_t             offset;
} BoardStateParseResult;
...
const PieceType*      PieceTypes_At(const PieceTypes* ts, size_t i);
PieceType*            PieceTypes_AtRef(PieceTypes* ts, size_t i);
